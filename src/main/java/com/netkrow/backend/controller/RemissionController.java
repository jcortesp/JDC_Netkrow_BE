...
public class RemissionController {
    ...
    @PostMapping
    public ResponseEntity<Remission> createRemission(@RequestBody RemissionRequest req) {
    ...
    public ResponseEntity<?> getRemission(@PathVariable String remissionId) {
        return service.getRemissionByRemissionId(remissionId)
                .map(r -> ResponseEntity.ok((Object) r))
                .orElse(ResponseEntity.status(404).body("Remisión no encontrada"));
    }
